...
$sql="SELECT A.*, B. `stuprogramname` as f1, `sfirstname` as f2, `slastname` as f3 FROM course as A 

LEFT JOIN `studentprogram` as B on A.`studentprogramID` = B.`studentprogramID`
LEFT JOIN `lecturer` as C on A.`lecturerID` = C.`lecturerID` 
LEFT JOIN `staff` as D on C.`staffID` = D.`staffID`";
