...
$sql="SELECT A.*, B. `progname` as f1, C.`durname` as f2 FROM studentprogram as A 

LEFT JOIN `schoolprogram` as B on A.`schoolprogramID` = B.`schoolprogramID`
LEFT JOIN `duration` as C on A.`durationID` = C.`durationID`";
$records=mysqli_query($link,$sql);
while ($row=mysqli_fetch_assoc($records)){

  echo "<tr>";
  
            echo "<td>".$row['studentprogramID']."</td>"; 
            echo "<td>".$row['stuprogramname']."</td>"; 
            echo "<td>".$row['f1']."</td>"; 
            echo "<td>".$row['f2']."</td>"; 
